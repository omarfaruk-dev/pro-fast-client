...
const Coverage = () => {
    ...
    const filteredLocations = coverageData.filter(({ district, city, covered_area }) => {
    ...
    return (
        <div className="bg-base-100 py-8 md:py-16 px-10 md:px-20 rounded-xl mb-8 md:mb-16">
            ...
            <MapContainer center={centerPosition} zoom={7} scrollWheelZoom={true} style={{ height: '500px', width: '100%' }}>
                <TileLayer
                    attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                />
                {filteredLocations.map(({ district, city, covered_area, latitude, longitude, flowchart }, index) => (
                    <Marker key={index} position={[latitude, longitude]} icon={customIcon}>
                        <Popup>
                            <div>
                                <h4 className="font-bold">{city} ({district})</h4>
                                <ul className="list-disc pl-4 text-sm">
                                    {covered_area.map((area, i) => (
                                        <li key={i}>{area}</li>
                                    ))}
                                </ul>
                                <img src={flowchart} alt={`${city} flowchart`} className="mt-2 w-full" />
                            </div>
                        </Popup>
                    </Marker>
                ))}
