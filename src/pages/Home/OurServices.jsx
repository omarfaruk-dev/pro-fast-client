...
const OurServices = () => {
  return (
    <section className="bg-[#0d3c3f] rounded-3xl py-12 px-4 md:px-10 my-15">
      <h2 className="text-white text-3xl md:text-4xl font-bold text-center mb-2">Our Services</h2>
      <p className="text-[#b6e0e2] text-center max-w-2xl mx-auto mb-10 text-base md:text-lg">Enjoy fast, reliable parcel delivery with real-time tracking and zero hassle. From personal packages to business shipments — we deliver on time, every time.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service, idx) => (
          ...
          >
            ...
            <p className={`text-gray-600 text-sm md:text-base ${idx === 1 ? 'text-[#5d6b5d]' : ''}`}>{service.description}</p>
          </div>
        ))}
      </div>
    </section>
